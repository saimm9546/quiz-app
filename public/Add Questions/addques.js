...
function renderoption(){
    ...
    for (let i = 0; i < options.length; i++) {
        optionparent.innerHTML += `<li onclick ="setcorrectans('${options[i]}')" class ="p-2 bg-success  fs-5 rounded shadow my-2" > ${options[i]} </li>`
...
window.submitquestion = function(){
    ...
    const refrence = ref(db ,`Question/${obj.id}`)
    set (refrence,obj)
